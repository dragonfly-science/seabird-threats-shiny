...
#############################
    ...
    all_dem_good <- reactive({
        ...
        if (!is.null(input$afr_lcl) & !is.null(input$afr_ucl) &
            ...
            !is.null(input$cs_min) & !is.null(input$cs_max)) {
            if (is.finite(input$afr_lcl) & is.finite(input$afr_ucl) &
                ...
                is.finite(input$cs_min) & is.finite(input$cs_max))
                go <- T
        } 
        return(go)
    })
    ...
    samples <- reactive({
        if (all_dem_good()) {
            s <- list(afr = samples_from_n_ci(input$afr_lcl, input$afr_ucl, input$nsamples),
                     ...
                     cs   = sample(seq.int(input$cs_min, input$cs_max, 1L), input$nsamples, replace=T))
            meanafr <- mean(s$afr)
    ...
    observeEvent(input$addthreat, {
        ...
        if (!is.null(input$threat) & !is.null(input$impacted_par) & !is.null(input$exist_pot)) {
            ...
            for (i in missing_changes) {
                ...
                tab[i, `Change` := scales[[par]] * `Individuals` / mean(pd[[param_n]])]
            }
            ...
            for (i in missing_indivs) {
                ...
                param <- pars_short[[par]]
                param_n <- rel_pars[[par]]
                tab[i, `Individuals` := (`Change` / scales[[par]]) * mean(pd[[param_n]])]
            }
...
#################
    ...
    summary_fun <- function(pd) {
        if (!is.null(pd)) {
            summ <- pd[, lapply(.SD, function(x) c(mean = mean(x),
...
#############
    ...
    output$downloadDemBtn <- downloadHandler(
        ...
        content = function(file) {
            demf <- copy(prepare_all_dem_summary())
            c <- intersect(names(demf), c('sic', 'si', 'sa', 'bs', 'pb'))
            for (v in c)  demf[, eval(v) := round(100 * get(v), 3)]
            demf$grate <- round((demf$grate - 1) * 100, 3)
            c <- setdiff(names(demf), c('stat', 'sic', 'si', 'sa', 'bs', 'pb', 'grate'))
            for (v in c) demf[, eval(v) := round(get(v))]
            demf[, stat := factor(stat, levels=unique(stat))]
            demf <- dcast(melt(demf, id.vars='stat'), variable ~ stat)
            setnames(demf, c('Parameter',
                             'Base-mean', 'Base-2.5-percentile', 'Base-97.5-percentile',
                             'Scenario-mean', 'Scenario-2.5-percentile', 'Scenario-97.5-percentile'))
            demf[, Parameter := labs[as.character(Parameter)]]
            fwrite(demf, file)
        }
    )
