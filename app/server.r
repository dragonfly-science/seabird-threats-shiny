...
all_dem_good <- reactive({
    ...
    if (!is.null(input$afr_lcl) & !is.null(input$afr_ucl) &
         ...
         !is.null(input$pb_lcl) & !is.null(input$pb_ucl) &
          !is.null(input$nbp_lcl) & !is.null(input$nbp_ucl)) {
        if (is.finite(input$afr_lcl) & is.finite(input$afr_ucl) &
            ...
            is.finite(input$nbp_lcl) & is.finite(input$nbp_ucl))
            go <- T
    } 
    return(go)
})
...
harvest_dem <- function() {
    if (!is.null(input$afr_lcl) & !is.null(input$afr_ucl) &
        ...
        parlist <- sapply(c('afr', 'sic', 'sa', 'bs', 'pb', 'nbp'), function(p)
            c(input[[paste0(p,'_lcl')]], input[[paste0(p,'_ucl')]]), simplify=F)
        d <- as.data.frame(do.call('rbind', parlist), stringsAsFactors=F)
        names(d) <- c('lcl', 'ucl')
        d$dem_par <- rownames(d)
        d$species <- input$spp
        d <- d[, c('species', 'dem_par', 'lcl', 'ucl')]
        rownames(d) <- NULL
        return(d)
    }
}
...
summary_fun <- function(spd) {
    if (!is.null(spd)) {
        summ <- as.data.frame(do.call('cbind', sapply(spd, function(x) {
            c(mean = mean(x),
...
prepare_all_dem_summary <- reactive({
    ...
    if (!is.null(dem1) & is.null(dem2))
        dem2 <- dem1
    if (!is.null(dem1) & !is.null(dem2)) {
        rownames(dem1) <- paste0(rownames(dem1), '_curr')
        rownames(dem2) <- paste0(rownames(dem2), '_threat')
        dem <- rbind(dem1, dem2)
        dem <- dem[, c('afr','sic','si','sa','bs','pb','nbp','ni','nad','ntot','grate','dead_ad','dead_juv')]
    }
    return(dem)
})

output$all_dem_summary <- DT::renderDataTable({
    dem <- prepare_all_dem_summary()
    if (!is.null(dem)) {
        c <- names(dem) %in% c('sic', 'si', 'sa', 'bs', 'pb')
        dem[, c] <- round(100 * dem[,c], 1)
        dem$grate <- round((dem$grate - 1) * 100, 2)
        c <- !(names(dem) %in% c('sic', 'si', 'sa', 'bs', 'pb', 'grate'))
        dem[, c] <- round(dem[, c])
        dem <- as.data.frame(t(dem))
        dem$lcl_ucl_curr <- sprintf('%s–%s', dem$lcl_curr, dem$ucl_curr)
        dem$lcl_ucl_threat <- sprintf('%s–%s', dem$lcl_threat, dem$ucl_threat)
        dem$lcl_curr <- dem$ucl_curr <- dem$lcl_threat <- dem$ucl_threat <- NULL
        dem$val_curr <- sprintf('%s  (%s)', dem$mean_curr, dem$lcl_ucl_curr)
        dem$val_threat <- sprintf('%s  (%s)', dem$mean_threat, dem$lcl_ucl_threat)
        dem$par <- labs[rownames(dem)]
        dem <- dem[, c('par', 'val_curr', 'val_threat')]
        colnames(dem) <- c('Parameter', 'Base', 'Scenario')
